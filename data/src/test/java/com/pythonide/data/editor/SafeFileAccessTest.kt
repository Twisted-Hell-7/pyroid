...
class SafeFileAccessTest {
    ...
    fun `test read non-existent file returns NotFound`() = runBlocking {
        val result = safeFileAccess.readFile(File(tempFolder.root, "nonexistent.txt").absolutePath)
    ...
    fun `test delete non-existent file returns NotFound`() = runBlocking {
        val result = safeFileAccess.deleteFile(File(tempFolder.root, "nonexistent.txt").absolutePath)
    ...
    fun `test list non-existent directory returns NotFound`() = runBlocking {
        val result = safeFileAccess.listDirectory(File(tempFolder.root, "nonexistent_dir").absolutePath)
